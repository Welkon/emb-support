        ...
        begin
            ...
            begin
                ...
                try
                    ...
                    AddJSONBoolean(ComponentProps, 'locked', not Component.Moveable);
                    AddJSONBoolean(ComponentProps, 'moveable', Component.Moveable);
                    AddJSONBoolean(ComponentProps, 'primitives_locked', Component.PrimitiveLock);
...
function DelimitedField(Value: String; FieldIndex: Integer): String;
var
    Rest: String;
    Part: String;
    SepPos: Integer;
...
function SetComponentPosition(Designator: String; NewX, NewY: Float; Rotation: Float; SkipIfLocked: Boolean = True): String;
var
    Board: IPCB_Board;
    Component: IPCB_Component;
    ResultProps: TStringList;
...
begin
    ...
    try
        ...
        begin
            ...
            AddJSONNumber(ResultProps, 'current_x', CoordToMils(Component.x - xorigin));
            AddJSONNumber(ResultProps, 'current_y', CoordToMils(Component.y - yorigin));
        ...
        AddJSONBoolean(ResultProps, 'locked', not Component.Moveable);
        AddJSONBoolean(ResultProps, 'moveable', Component.Moveable);
        AddJSONBoolean(ResultProps, 'primitives_locked', Component.PrimitiveLock);
...
var
    ...
    Component      : IPCB_Component;
    ResultProps    : TStringList;
    AppliedArray   : TStringList;
    SkippedArray   : TStringList;
...
begin
    ...
    ResultProps := TStringList.Create;
    AppliedArray := TStringList.Create;
    SkippedArray := TStringList.Create;
    ...
    try
        ...
        begin
            Designator := Trim(DelimitedField(PositionsList[i], 0));
            NewX := SafeStrToFloat(DelimitedField(PositionsList[i], 1));
            NewY := SafeStrToFloat(DelimitedField(PositionsList[i], 2));
            Rotation := SafeStrToFloat(DelimitedField(PositionsList[i], 3));
            ...
            else if SkipIfLocked and (not Component.Moveable) then
            begin
                ItemProps := TStringList.Create;
                try
                    AddJSONProperty(ItemProps, 'designator', Designator);
                    AddJSONProperty(ItemProps, 'reason', 'component-locked');
                    AddJSONBoolean(ItemProps, 'locked', True);
                    AddJSONBoolean(ItemProps, 'moveable', Component.Moveable);
                    AddJSONBoolean(ItemProps, 'primitives_locked', Component.PrimitiveLock);
                    AddJSONNumber(ItemProps, 'current_x', CoordToMils(Component.x - xorigin));
                    AddJSONNumber(ItemProps, 'current_y', CoordToMils(Component.y - yorigin));
                    SkippedArray.Add(BuildJSONObject(ItemProps, 1));
                finally
                    ItemProps.Free;
                end;
            ...
            begin
                ...
                try
                    AddJSONProperty(ItemProps, 'designator', Designator);
                    AddJSONNumber(ItemProps, 'new_x', NewX);
                    AddJSONNumber(ItemProps, 'new_y', NewY);
                    AddJSONNumber(ItemProps, 'rotation', Component.Rotation);
                    AppliedArray.Add(BuildJSONObject(ItemProps, 1));
                finally
                    ItemProps.Free;
                end;
        ...
        AddJSONInteger(ResultProps, 'missing_count', MissingCount);

        if (AppliedArray.Count > 0) then
            ResultProps.Add(BuildJSONArray(AppliedArray, 'applied'))
        else
            ResultProps.Add('"applied": []');

        if (SkippedArray.Count > 0) then
            ResultProps.Add(BuildJSONArray(SkippedArray, 'skipped'))
        else
            ResultProps.Add('"skipped": []');
