...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(str(Path(__file__).with_name('altium_pcb_live.log')))  # Also log to file
    ]
)
logger = logging.getLogger("AltiumPCBLiveServer")
...
# Both the Python live server and the Altium DelphiScript need to independently
...
EXCHANGE_DIR = Path("C:/Users/Public/altium_pcb")
...
class AltiumBridge:
    ...
    async def execute_command(self, command: str, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            response_text = None
            for encoding in ("utf-8", "utf-8-sig", "gbk", "cp936", "cp1252", "latin-1"):
                try:
                    response_text = response_bytes.decode(encoding)
                    logger.info(f"Decoded response with {encoding}")
                    break
                except UnicodeDecodeError:
                    continue
            if response_text is None:
                return {"success": False, "error": "Unable to decode response from Altium"}
...
def unwrap_bridge_result(response: Dict[str, Any]) -> tuple[Any, Optional[str]]:
    """Unwrap bridge responses that may contain nested script success fields."""
...
@mcp.tool()
async def set_component_position(
    ctx: Context,
    cmp_designator: str,
    x: float,
    y: float,
    ...
    Args:
        ...
        skip_if_locked (bool): If true, do not move locked PCB components
    
    Returns:
        str: JSON object with the result of the position operation
    """
    logger.info(
        f"Setting component {cmp_designator} position to X:{x}, Y:{y}, "
        f"Rotation:{rotation}, SkipIfLocked:{skip_if_locked}"
    )
    ...
    result, error_msg = unwrap_bridge_result(response)
    if error_msg:
...
@mcp.tool()
async def set_component_positions(
    ctx: Context,
    positions: list,
    skip_if_locked: bool = True,
    ...
    """
    Set multiple components to absolute PCB positions in one Altium script run

    Args:
        positions (list): Objects with designator/cmp_designator, x, y, and optional rotation in mils/degrees
        skip_if_locked (bool): If true, do not move locked PCB components
        stop_on_error (bool): If true, stop processing after a missing or invalid component

    Returns:
        str: JSON object with moved, skipped, and missing component results
    """
    normalized_positions = []
    for item in positions or []:
        if not isinstance(item, dict):
            continue
        designator = item.get("designator") or item.get("cmp_designator")
        if not designator:
            continue
        try:
            x = float(item.get("x"))
            y = float(item.get("y"))
            rotation = float(item.get("rotation", -1))
        except (TypeError, ValueError):
            logger.warning(f"Skipping invalid component position item: {item}")
            continue
    ...
    logger.info(
        f"Setting {len(normalized_positions)} component positions, "
        f"SkipIfLocked:{skip_if_locked}, StopOnError:{stop_on_error}"
    )
    ...
    result, error_msg = unwrap_bridge_result(response)
    if error_msg:
        logger.error(f"Error setting component positions: {error_msg}")
        return json.dumps({"success": False, "error": f"Failed to set component positions: {error_msg}"})

    logger.info("Component positions set successfully")
    return json.dumps({"success": True, "result": result}, indent=2)
...
async def get_server_status(ctx: Context) -> str:
    """Get the current status of the embedded Altium live server"""
    status = {
        "server": "Running",
...
if __name__ == "__main__":
    logger.info("Starting Altium PCB live server...")
