// altium_bridge.pas
// This script acts as a bridge between the altium-pcb live server and Altium
...
{ Both the Python live server and this script independently resolve to         }
{ C:\Users\Public\altium_pcb\ - avoiding fragile script-project-path          }
{ resolution that breaks when Altium caches stale script projects.             }
{..............................................................................}
procedure InitializeFilePaths();
begin
    ROOT_DIR := 'C:\Users\Public\altium_pcb\';
...
function ParseJSONBooleanValue(Value: String; DefaultValue: Boolean): Boolean;
var
    Text: String;
begin
    Text := LowerCase(TrimJSON(Value));
    if (Text = 'true') or (Text = '1') or (Text = 'yes') then
        Result := True
    else if (Text = 'false') or (Text = '0') or (Text = 'no') then
        Result := False
    else
        Result := DefaultValue;
end;

function GetJSONLineValue(Line: String): String;
var
    ValueStart: Integer;
begin
    ValueStart := Pos(':', Line) + 1;
    Result := Copy(Line, ValueStart, Length(Line) - ValueStart + 1);
end;
...
begin
    ...
    try
        ...
        begin
            ...
            // Look for skip_if_locked
            else if (Pos('"skip_if_locked"', RequestData[i]) > 0) then
            begin
                ParamValue := GetJSONLineValue(RequestData[i]);
                SkipIfLocked := ParseJSONBooleanValue(ParamValue, True);
            end;
        end;
        
        if Designator <> '' then
        begin
            Result := SetComponentPosition(Designator, NewX, NewY, Rotation, SkipIfLocked);
        end
        else
        begin
...
var
    ...
    InPosition: Boolean;
begin
    PositionsList := TStringList.Create;
    Designator := '';
    NewX := 0;
    NewY := 0;
    Rotation := -1;
    ...
    try
        for i := 0 to RequestData.Count - 1 do
        begin
            if (Pos('"skip_if_locked"', RequestData[i]) > 0) then
            begin
                ParamValue := GetJSONLineValue(RequestData[i]);
                SkipIfLocked := ParseJSONBooleanValue(ParamValue, True);
            end
            else if (Pos('"stop_on_error"', RequestData[i]) > 0) then
            begin
                ParamValue := GetJSONLineValue(RequestData[i]);
                StopOnError := ParseJSONBooleanValue(ParamValue, False);
            end
            else if (Pos('"positions"', RequestData[i]) > 0) then
            begin
                InPositions := True;
            end
            else if InPositions and (Pos('{', RequestData[i]) > 0) then
            begin
                InPosition := True;
                Designator := '';
                NewX := 0;
            ...
            else if InPositions and InPosition and ((Pos('"designator"', RequestData[i]) > 0) or (Pos('"cmp_designator"', RequestData[i]) > 0)) then
            begin
                ParamValue := GetJSONLineValue(RequestData[i]);
                ParamValue := TrimJSON(ParamValue);
                ParamValue := StringReplace(ParamValue, '"', '', REPLACEALL);
            ...
            else if InPositions and InPosition and (Pos('"x"', RequestData[i]) > 0) then
            begin
                ParamValue := GetJSONLineValue(RequestData[i]);
                ParamValue := TrimJSON(ParamValue);
                NewX := SafeStrToFloat(ParamValue);
            end
            else if InPositions and InPosition and (Pos('"y"', RequestData[i]) > 0) then
            begin
                ParamValue := GetJSONLineValue(RequestData[i]);
                ParamValue := TrimJSON(ParamValue);
                NewY := SafeStrToFloat(ParamValue);
            end
            else if InPositions and InPosition and (Pos('"rotation"', RequestData[i]) > 0) then
            begin
                ParamValue := GetJSONLineValue(RequestData[i]);
                ParamValue := TrimJSON(ParamValue);
                Rotation := SafeStrToFloat(ParamValue);
            end
            else if InPositions and InPosition and (Pos('}', RequestData[i]) > 0) then
            begin
                if Designator <> '' then
                    PositionsList.Add(Designator + '|' + FloatToStr(NewX) + '|' + FloatToStr(NewY) + '|' + FloatToStr(Rotation));
                InPosition := False;
            end
            else if InPositions and (Pos(']', RequestData[i]) > 0) then
            begin
        ...
        begin
            Result := SetComponentPositions(PositionsList, SkipIfLocked, StopOnError);
        end
        else
        begin
            ShowMessage('Error: No positions found for set_component_positions');
            Result := '';
        end;
    finally
        PositionsList.Free;
